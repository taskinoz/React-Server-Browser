...
const Communities = ({desc}) => {
  ...
  if (desc.includes('discord.gg')){
    let link = desc.split('discord.gg')[1]
    link = link.split(" ")[0]
    communities.discord = {
  ...
  return (
    <>
      {(Object.keys(communities)).map(community => 
        <a href={communities[community].link} target="_blank">{communities[community].icon}</a>
      )}
    </>
    );
}
